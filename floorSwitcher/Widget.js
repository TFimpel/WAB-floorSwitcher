...
define(["dojo/_base/Deferred", "dijit/form/Textarea", "dojo/dom-attr", "dijit/registry", "dojo/dom-class", "esri/graphicsUtils", "dojo/query", "dojo/dom-style", "jimu/PanelManager", "dojo/_base/array", "dijit/form/CheckBox", "dojo/dom-construct", "dojo/dom", "dojo/on", "esri/tasks/query", "esri/tasks/QueryTask", 'dojo/_base/declare', 'jimu/BaseWidget', "dojo/NodeList-traverse", "dojo/domReady!"],
    function(Deferred, Textarea, domAttr, registry, domClass, graphicsUtils, query, domStyle, PanelManager, arrayUtil, CheckBox, domConstruct, dom, on, Query, QueryTask, declare, BaseWidget) {
        ...
        return declare([BaseWidget], {
            ...
            compareBySecondArrayElement: function(a, b) {
                if (a[1] === b[1]) {
                    return 0;
                } else {
                    return (a[1] < b[1]) ? -1 : 1;
            ...
            onOpen: function() {
                ...
                domConstruct.create("p", {
                    innerHTML: "<br>The <i>Each building</i> mode switches floor levels for individual buildings.<br><br>The <i>All buildings</i> mode switches the floor level for all University buldings. <br><br>The <i>SQL</i> mode allows you to set the floors to be displayed via a SQL query."
                }, "floorList");
                ...
                function addFloorsToSlider(results) {
                    ...
                    domConstruct.create("div", {
                        id: clickedBuildingNumber,
                        class: "buildingFloorContainer"
                    }, "floorList");

                    //create a div that holds the building name and number
                    var buildingInfo = clickedBuildingName + " (" + clickedBuildingNumber + ")"
                    domConstruct.create("div", {
                        innerHTML: buildingInfo + "<br>",
                        id: buildingInfo,
                        class: "buildingHeading"
                    }, clickedBuildingNumber);
                    ...
                    domConstruct.create("div", {
                        id: clickedBuildingNumber + "_functionContainer",
                        class: "buidlingFunctionContainer"
                    }, clickedBuildingNumber);
                    domConstruct.create("button", {
                        innerHTML: "Persist",
                        id: clickedBuildingNumber + "_persist",
                        class: "persistButton"
                    }, clickedBuildingNumber + '_functionContainer');
                    domConstruct.create("button", {
                        innerHTML: "Remove",
                        id: clickedBuildingNumber + "_remove",
                        class: "removeButton",
                        style: "display: none"
                    }, clickedBuildingNumber + '_functionContainer');
                    ...
                    on(dom.byId(clickedBuildingNumber + "_persist"), "click", function(e) {
                        ...
                        domConstruct.create("div", {
                            innerHTML: "<br>Select a building footprint on the map.<br><br>"
                        }, "floorList");
                        ...
                        domStyle.set(clickedBuildingNumber + "_persist", {
                            "display": "none"
                        });
                        domStyle.set(clickedBuildingNumber + "_remove", {
                            "display": ""
                        });
                    })
                    ...
                    arrayUtil.forEach(clickedBuildingFloors, function(data) {
                        ...
                        domConstruct.create("div", {
                            id: building_floor + "_row",
                            style: "display:inline; clear:both;"
                        }, clickedBuildingNumber, 1);

                        //create the select floor button
                        domConstruct.create("button", {
                            innerHTML: floor,
                            id: building_floor + "_select",
                            class: "floorButton"
                        }, building_floor + "_row");
                ...
                function buildFloorSwitcher(e) {
                    clickedBuildingNumber = e.graphic.attributes[buildingIDFieldname];
                    clickedBuildingName = e.graphic.attributes[buidlingNameFieldname];
                ...
                function addAllFloors(results2) {
                    console.log('now running addAllFloors')
                ...
                function storeAllFloors(results) {
                    console.log('now running storeAllFloors')
                    domConstruct.create("p", {
                        id: 'loadingMessage',
                        innerHTML: "<br>Now loading floors into the 'All buildings tab'...</b>"
                    }, "floorList");
                    setTimeout(function() {
                        domConstruct.destroy('loadingMessage');
                    }, 3000);

                    var l = []
                    for (var i = 0; i < results.features.length; i++) {
                        l.push(results.features[i].attributes[floorIDFieldname]);
                    }
                ...
                on(dom.byId("all"), "click", function(e) {
                    ...
                    //add a help message
                    domConstruct.create("div", {
                        innerHTML: "<br>Select the floor level to display.<br>"
                    }, "floorList");
                ...
                on(dom.byId("bySQL"), "click", function(e) {
                    ...
                    domConstruct.create("div", {
                        innerHTML: "<br>Set a SQL query (for example FLOOR LIKE '%B'):"
                    }, "floorList");
            ...
            onClose: function() {
                    ...
                    buildingsLayer = this.map.getLayer(this.map.graphicsLayerIds[this.config.buildingsLayerNumber]);
                    wallsLayer = this.map.getLayer(this.map.graphicsLayerIds[this.config.wallsLayerNumber]);
                    roomsLayer = this.map.getLayer(this.map.graphicsLayerIds[this.config.roomsLayerNumber]);
